...
const searchRule: Reducer<SearchRule, SearchRulesAction> = (state, action) => {
  switch (action.type) {
  case '@@poi-plugin-akashic-records-ex/ADD_SEARCH_RULE':
    return {
      baseOn: DataSource.Filtered,
      content: '',
    }
  case '@@poi-plugin-akashic-records-ex/SET_SEARCH_RULE_BASE':
    return {
      content: '',
      ...state,
      baseOn: action.val as number,
    }
  case '@@poi-plugin-akashic-records-ex/SET_SEARCH_RULE_KEY':
    return {
      baseOn: DataSource.Filtered,
      ...state,
...
const reducer: Reducer<SearchRulesState, SearchRulesAction> = (state, action) => {
  if (state == null) {
    state = [searchRule(undefined, {type: '@@poi-plugin-akashic-records-ex/ADD_SEARCH_RULE'})]
  }
  switch (action.type) {
  case '@@poi-plugin-akashic-records-ex/ADD_SEARCH_RULE':
